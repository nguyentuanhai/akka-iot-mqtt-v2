...
class IotManager(clusterClient: ActorRef, numOfDevices: Int, mqttPubSub: ActorRef) extends Actor with ActorLogging {
    ...
    override def preStart(): Unit = {
        ...
        (1 to numOfDevices).foreach { n =>
            ...
            val deviceActor = context.actorOf(
                    Device.props(deviceType, deviceId, mqttPubSub),
    ...
    def ready: Receive = {
        ...
        case msg: Message => {
            ...
            log.info("IoT Agent -> Received MQTT message: {}-{} | State {} | Setting {}",
                work.deviceType, work.deviceId, work.currState, work.currSetting)
        ...
        case result @ WorkResult(workId, deviceType, deviceId, nextState, nextSetting) =>
            idToActorMap.get(deviceId) match {
                case Some(deviceActor) =>
                    ...
                    log.info("IoT Manager -> Work result forwarded to {}-{} ",
                        deviceType, deviceId)
                case None =>
                    log.info("IoT Manager -> ALERT: {}-{} NOT in registry!",
                        deviceType, deviceId)
            }
        ...
        case Ok(work) =>
            log.info("IoT Manager -> ALERT: Receive ack from Master but Device Id of {}-{} NOT in registry!",
                work.deviceType, work.deviceId)

        case NotOk(work) =>
            log.info("IoT Manager -> ALERT: Did not receive ack from Master and Device Id of {}-{} NOT in registry!",
                work.deviceType, work.deviceId)
